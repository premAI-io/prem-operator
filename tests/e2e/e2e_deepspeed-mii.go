	...
	When("the config is good", func() {
		...
		It("creates the deployment", func() {
			...
			Eventually(func(g Gomega) bool {
				...
				g.Expect(c.StartupProbe.InitialDelaySeconds).To(Equal(int32(30)))
				g.Expect(c.StartupProbe.PeriodSeconds).To(Equal(int32(5)))
				...
				g.Expect(deploymentPod.Spec.ImagePullSecrets).ToNot(BeEmpty())
				g.Expect(deploymentPod.Spec.ImagePullSecrets[0].Name).To(Equal(constants.RegcredDockerhub))
